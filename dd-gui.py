...
# import modules
import os, os.path, sys, shutil, pathlib, requests, random
try:
    from PyQt5.QtCore import *
    from PyQt5.QtGui import *
    from PyQt5.QtWidgets import *
except:
    print("PyQt5 was not found.")
    sys.exit(1)

# dd path
try:
    DD_PATH = os.environ['DD_PATH']
except:
    DD_PATH = "dd"

# xzcat path
try:
    XZCAT_PATH = os.environ['XZCAT_PATH']
except:
    XZCAT_PATH = "xzcat"
...
try:
    ...
    DOWNLOAD_PATH = f"{os.environ['HOME']}/.dd-gui/downloads"
...
def write_start():
    if not os.path.exists(INPUT_IF.text()) and not "http://" in INPUT_IF.text() and not "https://" in INPUT_IF.text():
    ...
    if "http://" in INPUT_IF.text() or "https://" in INPUT_IF.text():
        SUFFNX = pathlib.Path(INPUT_IF.text()).suffix
        if SUFFNX == ".xz":
            SUFFX = ".iso.xz"
        if SUFFNX == ".lzma":
            SUFFX = ".iso.lzma"
        ...
        print(f"Downloading {INPUT_IF.text()}...")
        di = requests.get(INPUT_IF.text(), allow_redirects = True)
        RAND_ISO = f"{DOWNLOAD_PATH}/{random.randint(1,1000)}{SUFFX}"
        open(RAND_ISO, "wb").write(di.content)
        INPUT_IF.setText(RAND_ISO)
        STATUS = "idle"
        TEXT_STATUS.setText(f"Status: {STATUS}")
    
    # if pathlib.Path(INPUT_IF.text()).suffix == ".xz" or pathlib.Path(INPUT_IF.text()).suffix == ".lzma":
    if ".img.xz" in INPUT_IF.text() or ".iso.xz" in INPUT_IF.text() or ".raw.xz" in INPUT_IF.text() or".img.lzma" in INPUT_IF.text() or ".iso.lzma" in INPUT_IF.text() or ".raw.lzma" in INPUT_IF.text():
        EXEC_CMD = f"{XZCAT_PATH} '{INPUT_IF.text()}' | {DD_PATH} of='{INPUT_OF.text()}' {WRITE_BS} {WRITE_COUNT} {WRITE_SKIP} {WRITE_CONV} status=progress"
    else:
        EXEC_CMD = f"{DD_PATH} if='{INPUT_IF.text()}' of='{INPUT_OF.text()}' {WRITE_BS} {WRITE_COUNT} {WRITE_SKIP} {WRITE_CONV} status=progress"
    
    print(f"Command to be executed: {EXEC_CMD}")
    
    if "/dev/" in INPUT_OF.text():
        if not os.path.exists(INPUT_OF.text()):
            print("Device file does not exist")
            sys.exit(1)
        BUTT_WARN = QMessageBox.question(WINDOW, "WARNING", f"You want to write to {INPUT_OF.text()} device! Make sure it's the correct one and that you have root access!", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if BUTT_WARN == QMessageBox.No:
            print("User aborded.")
            sys.exit(0)
...
def main():
    ...
    global WINDOW, WIDTH, HEIGHT, CAPTION, INPUT_IF, INPUT_OF, STATUS, CB_BS, CB_COUNT, CB_SKIP, TEXT_CONV, INPUT_BS, INPUT_COUNT, INPUT_SKIP, DRP_CONV, TEXT_STATUS, BUTT_WARN
    
    WIDTH = 500
    HEIGHT = 200
    CAPTION = "dd-gui v0.7.3"
    STATUS = "idle"
...
if __name__ == '__main__':
    if not os.path.exists(DOWNLOAD_PATH):
        print(f"Create {DOWNLOAD_PATH}")
        os.makedirs(DOWNLOAD_PATH, 775)
    main()
